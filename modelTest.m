function [R2, er, error] = modelTest(meanPSF_G, Gcamp, Rcamp)
...
%figure; imagesc(psf1)
% 
psf2=vertcat(meanPSF_G((sqSz+1):(1.5*(sqSz)-.5)),0,meanPSF_G((1.5*(sqSz)+.5):length(meanPSF_G)-1));
psf2=psf2';
...
% pix=length(GcampZs)/length(Gcamp);
% GcampZs=reshape(GcampZs,length(Gcamp),pix);
% meanPSF=reshape(meanPSF,length(Gcamp),pix,338);
% meanPSF=meanPSF(1:end-1,:,:);
% GcampZs=GcampZs(2:end,:);
% GcampZs=reshape(GcampZs,(length(Gcamp)-1)*pix,1);
% meanPSF=reshape(meanPSF,(length(Gcamp)-1)*pix,338);
