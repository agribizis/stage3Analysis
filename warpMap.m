...
for i = 1:length(V1pix)
    ...
    if r-fS>0 && c-fS>0 && min(min(V1mask(r-fS:r+fS, c-fS:c+fS,:))) ~=0
        V1ind(end+1)=i;
    end
end
SCpix=SCpix(V1ind',:);
...
for i=1:size(SCpix,1)
 %[r c]=SCpix(i,:);
        ...
        tmp=reshape(tmp, sz(3), (fS*2+1)*(fS*2+1));
 %[r2 c2]=V1pix(i,:);       
        tmp2=nV1(V1pix(i,1)-fS:V1pix(i,1)+fS, V1pix(i,2)-fS:V1pix(i,2)+fS,:);
        tmp2=reshape(tmp2, sz(3), (fS*2+1)*(fS*2+1));
        
        %180605 recently changed code so that it wouldn't have to take the
        %prime of each array
